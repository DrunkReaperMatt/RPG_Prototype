...
#include "Slate/SEdNode_StateMachineNode.h"
...
void SEdNode_StateMachineNode::AddPin(const TSharedRef<SGraphPin>& PinToAdd)
{
	...
	if (const bool bAdvancedParameter = PinObj && PinObj->bAdvancedView)
	{
...
bool SEdNode_StateMachineNode::IsNameReadOnly() const
{
	...
	UStateMachineBase* GenericGraph = EdNode_Node->GraphNode->Graph;
	check(GenericGraph != nullptr);

	return (!GenericGraph->bCanRenameNode || !EdNode_Node->GraphNode->IsNameEditable()) || SGraphNode::IsNameReadOnly();
}
